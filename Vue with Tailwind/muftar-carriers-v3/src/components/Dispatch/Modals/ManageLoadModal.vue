    ...
    <div>
        <div class="overflow-x-auto overflow-y-auto fixed shadow-lg inset-0 z-50 flex justify-center items-center">
            <div class="relative mx-auto   max-w-2xl">
                <div class="bg-white justify-center  relative mx-auto shadow-lg rounded-xl" ref="el">
                                ...
                                <div v-if="current_load.status == 'pending_confirmation'">
                                    ...
                                    <div class="my-4" v-if="bol_list.length > 0">
                                        ...
                                        <div class="grid mt-2" v-if="listView">
                                            ...
                                            <div v-for="(bl, idx) in bol_list" :key="idx">
                                                <a :href="bl.filepath" class="grid grid-cols-2   rounded-md  text-center"
                                                    target="_blank">
...
export default {
    name: 'ManageLoadModal',
    props: ['current_load'],
    data() {
        return {
            loading: false,
            rejectionState: false,
            bol_list: [],
            current_driver: { load_assignment: { load: '' } },
            activeDriver: false,
            // current_load: {},
            activeLoadConfirmId: "",
            post: '',
    ...
    computed: {
        ...mapState(['orgID']),
        loadID() {
            return this.current_load.id
        },
        driverID() {
            return this.current_load.assignment.driver
        }
    },
    methods: {
        async unAssignLoadFromDriver() {
            const config = {
                headers: {
                    orgID: this.orgID,
            ...
            const payload = {
                loadID: this.current_load.id,
                driverID: this.driverID
            }
            console.log(payload)
            try {
                const res = await axios.post(this.$api_path + "/loads/unassign_driver_from_load", payload, config)
                console.log("DRIVER UNASSIGNED", res.data);
                if (res.data.status === 'warning') {
                    this.feedback.status = res.data.status;
                    this.feedback.text = res.data.feedback;
                } else if (res.data.status === 'success') {
                    this.hideModal()
                    this.$emit('reCallLoads')
                }
                // console.log(respoonse.data)
                //const data=res.data.status
                //this.post=data;
                // console.log(this.post,'asdasdasd')
                // commit('MOVE_ASSIGN_LOAD',loadIndex)
                this.loading = false;
        ...
        viewOnGoogleMaps(coords) {
            // window.open(`https://www.google.com/maps/@${coords.latitude},${coords.longitude},15z`)
            window.open(`https://www.google.com/maps/search/?api=1&query=${coords.latitude}%2C${coords.longitude}`)

        },
        async makeBOlDecision(choice) {
            let confirmBol = false;
            this.loading = true;
            if (choice == 'accept') {
                confirmBol = true;
            ...
            const payload = {
                ...
                bol_list: this.bol_list,
            }
            console.log(payload)
            try {
                const res = await axios.post(this.$api_path + "loads/bol/decision", payload, config);
                console.log(res.data);
                // this.loading = false
                this.$emit('reCallLoads')
                this.$emit('hideModal')
        ...
        async getDriver() {
            ...
            try {
                const res = await axios.get(this.$api_path + "/fleet/drivers/one", config)
                this.activeDriver = true;
                this.current_driver = res.data.result;
        ...
        async getBOLs() {
            ...
            try {
                const res = await axios.get(this.$api_path + "loads/bols", config);
                // this.current_driver=res.data.result
                this.bol_list = res.data.result;
                console.log(res.data)
                this.loading = false;
            } catch (error) {
                console.log(error);

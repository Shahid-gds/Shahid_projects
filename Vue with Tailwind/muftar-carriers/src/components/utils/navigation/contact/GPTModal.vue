<template>
  ...
  <div class="fixed h-[800px] bg-white z-50 top-0 left-[30%]  rounded-xl shadow  w-[708px] " ref="el">
    ...
    <div class="container mx-auto p-4 w-full h-full bg-[white] rounded-xl">
      ...
      <div class="p-4">
        <div
          class="p-2 flex justify-center cursor-pointer space-x-3 items-center border-2 rounded-xl bg-[#FBFBFB] border-[#D6D6D6] px-4 outline-none"
          @click="newChat">
          <div class="">
                ...
                <div class="w-full justify-between">
                  ...
                  <div class="flex items-center justify-end space-x-2 mt-2" v-if="!message.loading">
                    <button @click="likeMessage(message)" class="text-green-500">
                      <svg :class="{ 'text-black': message.liked }" xmlns="http://www.w3.org/2000/svg" fill="none"
                    ...
                    <button @click="dislikeMessage(message)" class="text-red-500">
                      <svg :class="{ 'text-black': message.disliked }" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div>
                      <button @click="showReviewModal = true" class="border-2 p-1 px-3 rounded-lg">Review</button>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div class="input-container bg-[#FFFFFF] items-center mx-auto p-4 justify-between rounded-b-xl">
        <div class="p-2 flex items-center justify-between">
          <input type="text"
            ...
            placeholder="Ask Muftar GPT" autocomplete="off" v-model="inputText" @input="handleInput"
            @keydown.enter="sendMessage" @keydown.esc="hideList" />
          <div id="listContainer" class="bottom-28 p-4 rounded-lg h-72 overflow-y-auto" v-if="showList">
            <ul class="py-3">
              <li class="list-item py-1.5 px-2 cursor-pointer hover:bg-gray-300 hover:rounded-lg"
                v-for="item in currentList" :key="item" @click="selectItem(item)">
...
export default {
  ...
  methods: {
    async closeModal() {
      try {
        const response = await axios.post('http://127.0.0.1:5000/end', {
    ...
    async sendMessage() {
      ...
      const message = {
        ...
        driverID:'640fd2d4eeec07d7f94092d1',
        loadID: '6419da23fe3a41a21e69bae6'
      };
      ...
      try {
        const response = await axios.post('http://127.0.0.1:5000/user_ask', message);
    ...
    handleInput() {
      ...
      if (inputText.endsWith('@')) {
        ...
        fetch('http://127.0.0.1:5000/unassingned_loads', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
...
#listContainer {
  ...
  z-index: 998;
  /* Lower z-index for #listContainer */
}

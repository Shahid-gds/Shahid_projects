<template>
    <div class="flex flex-col px-8" style="min-height:80vh;">
        <div class="flex mt-2 items-center">
            <!-- <div>
                <DropdownAnimated @selectedItem="getStatements" :options="updated_drivers" :loading="drivers_loading"
            ...
            <div v-if="statements.length > 0" class="ml-2 relative">
                <div v-if="current_statement != null" class="p-2 border rounded-md">
                    {{ Object.keys(current_statement.details).length > 0 ?
                        $fmtDate(current_statement.details.pay_period.start_time) : "00-00-00" }} - {{ current_statement.details
        ?
        $fmtDate(current_statement.details.pay_period.end_time) : "00-00-00" }}
                </div>
                <div class=" flex flex-col space-y-2 w-full p-2 border rounded-md cursor-pointer"
                    ...
                    <div class="absolute bg-white top-4 w-full border rounded-md p-2" v-if="openMenu" @click="setStatement(stmt)">
                        {{ Object.keys(stmt.details).length > 0 ? $fmtDate(stmt.details.pay_period.start_time) : "00-00-00"
                        }}
            ...
            <!-- <div v-if="statements.length > 0" class="flex">
                <div v-for="(stmt, idx) in all_statements" :key="idx" @click="setStatement(stmt)"  :class="(current_statement && current_statement.id == stmt.id) || ($route.query.statement_id == stmt.id)
                ...
                ">
                    <p
                        v-if="(current_statement && stmt.id == current_statement.id) || ($route.query.statement_id == stmt.id)">
                        *</p>
                    <p>

                        {{
                            Object.keys(stmt.details).length > 0
                    ...
                    <p class="text-sm text-gray-500 italic absolute bottom-0 right-3">
                        {{ stmt.id }}
                    </p>
                </div>
            </div> -->

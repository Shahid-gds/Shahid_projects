<template>
  ...
  <div class=" justify-between border-gray-400 border-2 p-2 my-4">
    ...
    <div class="flex jusitfy-between w-full mt-5 border-b-2 border-t-2 pb-3 border-gray-500 border-t-2 border-b-2">
    ...
    <div class="flex">
      <table class="w-1/2">
        ...
        <tr>
          <td class="font-bold">Value</td></tr>
      </table>

      <table class="w-1/2">
        <tr><th>Pay Code</th> <th> Pay Type</th> <th>Rate</th> <th>Total</th></tr>
        <tr v-for="(tx, idx) in load_data.stats.table" :key="idx">
          ...
          <td class="text-blue-600">{{(tx.total).toLocaleString()}}</td></tr>
        <tr>
          <td class="text-blue-600"></td>
          <td class="text-blue-600"></td>
          <td class="font-bold">Total</td>
          <td class="text-blue-600">${{(totalAmount).toLocaleString()}}</td></tr>
      </table>
    </div>
  </div>
</template>

<template>
  <div class="container mx-auto fixed lg:h-[820px] bg-white z-50 top-0 lg:left-[30%]  rounded-xl shadow  lg:w-[708px] " ref="el">
      ...
      <div class="container mx-auto p-4 w-full h-full bg-[white] rounded-xl">
          ...
          <div class="p-4 flex space-x-2">
              <div class="p-2 w-full flex justify-center cursor-pointer space-x-3 items-center border-2 rounded-xl bg-[#FBFBFB] border-[#D6D6D6] px-4 outline-none"
                  @click="newChat">
                  <div class="">
              ...
              <div class="border-2 p-2 rounded-xl flex hover:bg-[red] hover:text-[white] cursor-pointer" @click="handleClick">
              ...
              <div class="message" v-for="(message, index) in chatMessages" :key="index">
                  <div class="message-content">
                      <div class="flex items-start space-x-6 p-2" v-if="message.sender === 'user'">
                        <div class="bg-[#BB5959] rounded-full p-2 top-0">
                          <!-- <h1 class="w-10 h-10 text-center py-2 text-white text-xl">SK</h1> -->
                          <img src="../assets/images/user_img.svg" alt="" class="w-10 h-10">
                        </div>
                          <div class="" v-if="user">
                      ...
                      <div v-else>
                          <div class="flex items-start space-x-6  w-full bg-[#EEF2FE] rounded-lg p-2">
                            <div class="bg-[#333333] rounded-full p-2 px-3" v-if="!message.loading">
                              <img src="../assets/images/chatwhitelogo.svg" alt="" class="w-10 h-10">
                            </div>
...
export default {
  ...
  window.addEventListener('beforeunload', async (event) => {
    if (!unloadEventFired) {
      ...
      try {
        const response = await axios.post('hhttps://muftar-gpt-mfa4wusola-uc.a.run.app/end', {
          orgID: "",
          userID: this.user.uid,
        });
...
const message = {
  orgID: "6408e67d600cfc60dbda1a6e",
  userID: this.user.uid,
...
try {
  ...
  const metadata = response.data.conversational_metadata.slice(-1)[0];
  console.log('AI Reponse:', metadata.displayed_response)
  const newMessageWithResponse = {
    content: metadata.displayed_response,
  ...
  fetch('https://muftar-gpt-mfa4wusola-uc.a.run.app/drivers', {
    ...
    .then(drivers => {
      ...
      this.currentList = drivers.map(driver => {
        ...
        return `${firstName} ${lastName} (ID: ${hiddenId})`;
      });
      this.showList = true;
    })
    .catch(error => {
  ...
  fetch('https://muftar-gpt-mfa4wusola-uc.a.run.app/unassingned_loads', {
    ...
    .then(unassignLoads => {
      ...
      this.currentList = unassignLoads.map(load => {
        ...
        // console.log('Load Id:', hiddenLoadId)
        return `${''}${hiddenLoadId} (ID: ${''}) (ID: ${loadId})`;
      });
      this.showList = true;
    })
    .catch(error => {
